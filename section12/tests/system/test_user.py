...
def test_register_new_user(test_client_db):
    ...
    res = test_client_db.post("register", data={"username": "testname", "password": "testpwd"})
...
def test_register_duplicated_user(test_client_db):
    test_client_db.post("register", data={"username": "testname", "password": "testpwd"})
    res = test_client_db.post("register", data={"username": "testname", "password": "testpwd"})
...
def test_user_valid_login(test_client_db):
    ...
    res = test_client_db.post("login", data={"username": "testname", "password": "testpwd"})
    assert res.status_code == 200
...
def test_user_invalid_login(test_client_db):
    ...
    test_client_db.post("register", data={"username": "testname", "password": "testpwd"})
    res = test_client_db.post("login", data={"username": "testnamewrong", "password": "testpwd"})
    assert res.status_code == 401
    assert json.loads(res.data) == {"msg": "Invalid credentials."}

    res = test_client_db.post("login", data={"username": "testname", "password": "testpwdwrong"})
    assert res.status_code == 401
    assert json.loads(res.data) == {"msg": "Invalid credentials."}
...
def test_user_logout(test_client_db):
    ...
    # kterej se logne
    auth_res = test_client_db.post("login", data={"username": "testname", "password": "testpwd"})
    ...
    res = test_client_db.post("logout", headers={"Authorization": f"Bearer {auth_token}"})
    assert res.status_code == 200
...
def test_refresh_token(test_client_db):
    ...
    res = test_client_db.post("refresh", headers={"Authorization": f"Bearer {refresh_token}"})
    assert res.status_code == 200
    assert json.loads(res.data)["access_token"] != ""
